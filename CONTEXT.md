...
_Avoid_: benchmark, test suite, dataset

**Corpus version**:
The identity of the benchmark state a score was produced against, so a threshold pass is unambiguously tied to its corpus. Decided in #20: a **content hash** over the normalized (sorted-key) `input.json` + `expectations.json` of every case, sorted by case-id, computed at eval time and stamped into the run record next to model and capability version. No manual bumping — adding or editing a case changes the hash, correctly marking it a different corpus. Per-case versioning is unnecessary.
_Avoid_: benchmark version, semver, git sha

**Deterministic evaluator assertions**:
What the deterministic evaluator checks a `WorkoutReview` against, given no model judgment (decided in #20). **Always-on** (not per-case): every cited Evidence row resolves **verbatim** to a row in the case's `input.json` (*no invented facts*), and ≤3 observations each with ≥1 Evidence. **Per-case** (from `expectations.json`): all `required_evidence` cited, all `required_limitations` present, and no observation whose `(category, derived-exercise)` matches a `forbidden_observations` entry. Prose quality/clarity is out of scope here — that is the deferred LLM rubric.
_Avoid_: rubric, scorer, grader

**Passing threshold**:
The per-metric aggregate floors a benchmark run must clear to **pass** (decided in #33, implemented #37 in `workout_review/threshold.py`). *Not* a cases-passing count — that number is informational only. Three tiers: **safety** (zero-tolerance) — `evidence_validity_rate == 1.0` and `unsupported_claim_rate == 0.0`, carrying the "no invented facts" promise; **structural** — `schema_valid_rate == 1.0`; **quality** (revisable, ratchet up over tune-loop) — `required_detail_recall ≥ 0.60` and `required_limitation_recall ≥ 0.33`. `required_limitation_recall` is **micro-averaged over only the cases that require a limitation** so it tracks the data-quality-limitation failure mode rather than diluting it. The quality floors sit below the observed 0.2.0 spread (detail clusters ~0.66, low 0.632; the first-recorded 0.708 was a high-tail single run) since a quality floor gets no retest. A failed **safety** floor triggers **confirm-before-failing**: each offending case is re-generated up to `SAFETY_RETEST_N` (3) times and the floor fails only if the violation *reproduces* — a clean sweep clears it as a flake. This is not retry-until-pass. The verdict is stamped into the run artifact under `gate`.
_Avoid_: pass rate, cases passing, retry-until-pass
