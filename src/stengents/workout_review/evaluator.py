...
# --------------------------------------------------------------------------- #
# Deterministic resolution of evidence against the pool
# --------------------------------------------------------------------------- #
...
def _sessions_by_id(pool: Iterable[dict]) -> dict[str, dict]:
    return {session["id"]: session for session in pool if "id" in session}


def _set_rows_by_session(pool: Iterable[dict]) -> dict[str, dict[tuple[str | None, int], dict]]:
...
def _resolve_fact_evidence(row: FactEvidence, sessions: dict[str, dict]) -> bool:
    ...
    if row.exercise is None:
        ...
        if row.field == "date":  # accept the full ISO stamp or its date prefix
            return row.value == _as_text(source) or row.value == _as_text(source)[:10]
        return row.value == _as_text(source)
    ...
    if row.field == "planned_sets":
        ...
        if prescribed is None:
            return False
        return row.value == _as_text(len(prescribed))
...
def _resolves(row: Evidence, set_index: dict, sessions: dict[str, dict]) -> bool:
