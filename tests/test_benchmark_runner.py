...
def test_parse_recognizes_write_baseline_flag() -> None:
    positional, model_override, write_baseline_flag, port_override, *_ = cli._parse(["review-benchmark", "--write-baseline"])
...
def test_parse_recognizes_serve_coach_port() -> None:
    positional, model_override, _, port_override, *_ = cli._parse(["serve-coach", "--port", "9001", "--model", "qwen2.5:7b-8k"])
    assert positional == ["serve-coach"]
    assert model_override == "qwen2.5:7b-8k"
    assert port_override == 9001

    _, _, _, unflagged_port, *_ = cli._parse(["serve-coach"])
